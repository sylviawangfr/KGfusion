...
class PerEntDegreeDataset(FusionDataset):
    ...
    def _get_out_degree_eval_scores(self, model_h_ent_eval, h_ent2idx):
    ...
    def _get_in_degree_eval_scores(self, model_t_ent_eval, t_ent2idx):
    ...
    def generate_pred_input_feature(self):
        ...
        for m in self.context_loader.models:
            m_preds = self.context_loader.load_preds([m])
            ...
            h_ent_eval.append(self._get_out_degree_eval_scores(m_degree_eval[m]['out_degree_eval'], m_degree_eval['out_id2eval_idx']))  # tail predicition
            t_ent_eval.append(self._get_in_degree_eval_scores(m_degree_eval[m]['in_degree_eval'], m_degree_eval['in_id2eval_idx']))  # head predicition
        ...
        h_features = torch.concat([ent_h_eval_feature, head_scores], 1)
        t_features = torch.concat([ent_t_eval_feature, tail_scores], 1)
